...
class SNCOAT_Env_v2(PyRep):
    def __init__(self, name='SNCOAT_Env', scene_path='', headless=True, history_len=4,
                 ...
                 action_type='Discrete', state_type='Image', image_type='Color', actuator_type='Force',
        ...
        self.state_types = ['Image', 'Pose', 'PoseImage', 'PosImage', 'OrientImage']
        ...
        if self.action_type == 'Discrete':
            ...
            self.action_space = spaces.Box(low=np.array([-1, -1, -1, -1, -1, -1]),
                                           high=np.array([1, 1, 1, 0, 0, 0]), dtype=float)
        else:
    ...
    def reset(self, is_record=False) -> dict:
        ...
        state_dict = {
            'image': np.array(stack_state),
            'position': self.target_pos_in_camera.astype(np.float32),
            'orientation': self.target_ang_in_camera.astype(np.float32),
            'pose': np.concatenate([self.target_pos_in_camera.astype(np.float32),
                                    self.target_ang_in_camera.astype(np.float32)])
        }
    ...
    def env_step(self, action):
        ...
        for _ in range(self.history_len):
            ...
            if self.actuator_type == 'Force':
                ...
                new_chaser_motion_vel = action[:3]
...
def run(thread_num):
    ...
    for i in range(5):
        ...
        env = SNCOAT_Env_v2(name='scene_{:02d}'.format(thread_num + 1), scene_path=scene_path, log_dir='log',
                            action_type='Discrete', image_type='Color', actuator_type='Velocity',
                            clear_record=True, headless=True, wait_done_steps=10, history_len=1,
                            actuator_noise=False, time_delay=False, image_blur=False, blur_level=1)
        env.run(episode_num=1)
        # env.stop()
        env.shutdown()


def run_agent(agent, episode_num=5):
    scenes_paths = sorted(glob.glob(os.path.join(TEST_SCENES_DIR, '*.ttt')))
    if len(scenes_paths) > 0:
        print('=> {} scenes file found in {}'.format(len(scenes_paths), TEST_SCENES_DIR))
    else:
    ...
    # for i in range(5):
    #     thread_num = i
    scene_idx = np.random.randint(0, len(scenes_paths))
    scene_path = scenes_paths[scene_idx]
    print('=> reloading scene in {} ...'.format(scene_path))
    env = SNCOAT_Env_v2(name='scene_{:02d}'.format(scene_idx + 1), scene_path=scene_path, log_dir='log',
                        action_type='Discrete', image_type=agent.image_type, actuator_type='Velocity',
                        clear_record=True, headless=True, wait_done_steps=10, history_len=1,
                        actuator_noise=False, time_delay=False, image_blur=False, blur_level=1)
    ...
    for t in range(episode_num):
        rewards = []
        if t % 1 == 0:
            state_dict = env.reset(True)
        else:
            state_dict = env.reset(False)
        # print('=> {}th step, state shape: {}'.format(self.step_count, state.shape))
        state_dict = agent.state_transform(state_dict, env)
        start_time = time.time()
        while True:
            action_idx, action = agent.choose_action(state_dict, env)
            state_dict, reward, done, _ = env.env_step(DISCRETE_ACTIONS[action_idx])
            state_dict = agent.state_transform(state_dict, env)
            # state, reward, done, _ = self.env_step(DISCRETE_ACTIONS[0])
            # print(reward)
            rewards.append(reward)
            if done or env.step_count > 1000:
                print('=> episode: {}, episode length: {}, episode rewards: {}'.format(
                    t + 1, env.step_count, np.sum(rewards)
                ))
                env.save_records()
                break
        end_time = time.time()
        elapsed_time = end_time - start_time
        print('=> speed for one episode with {} timesteps: {:0.2f} Hz'.format(
            env.step_count, env.step_count / elapsed_time))
...
if __name__ == '__main__':
    ...
    run(0)
    # drqn = DRQN(name='drqn', state_type='Pose', image_type='RGBD', action_type='Discrete',
    #             actuator_type='Velocity', attention_type='No', action_dim=len(DISCRETE_ACTIONS), gpu_idx=0,
    #             replay_buffer_size=50000, batch_size=32, episode_len=1000, lr=1e-4, data_augs=['No'],
    #             update_interval=1000, min_history=4, state_to_update=8, lstm_dim=128, lstm_layers=2, head_num=8,
    #             is_training=False, with_SE=True, vis=False)
    # run_agent(drqn, episode_num=5)
